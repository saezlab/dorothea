...
#' \code{\link[=viper]{viper()}} such as \code{minsize} or \code{method}.
#'  These options should not include, \code{eset} or \code{regulon}.
...
#'  other inputs the function will return a matrix. If \code{tidy} is \code{TRUE}
#'  the normalized enrichment scores are retured in a tidy format (not supported
#'  for Seurat objects).
#'
#' @export
#' @import dplyr
...
run_viper.ExpressionSet = function(input, regulons, options = list(), tidy=F) {
  run_viper(exprs(input), regulons = regulons, options = options, tidy = tidy)
}
