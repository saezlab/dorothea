...
pbmc <- readRDS("../inst/extdata/for_vignette/seurat_object.RDS")
```
...
## Estimate TF activity at the single-cell level

We first run the **VIPER** method on **DoRothEA's** regulons to obtain TFs activity,
by using *DoRothEA's* wrapper function `run_viper()`. This function can deal with 
...
We kindly refer the user to [decoupleR](https://github.com/saezlab/decoupleR) which provides a wide range of methods
and best practices for enrichment analysis, also available in [Python](https://github.com/saezlab/decoupler-py).
decoupleR is described in detail [here](https://doi.org/10.1093/bioadv/vbac016)).
...
**DoRothEA's** regulons. 
Note that in this case we use the scaled data by  rows, thus this allows us to distinguish TFs across cells.
One could also run  the enrichment analysis on the normalized (non-scaled) counts,
depending on the goals of the analysis of question.
