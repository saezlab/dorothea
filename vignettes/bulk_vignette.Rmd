--- 
title: "TF activity inference from bulk transcriptomic data with DoRothEA as regulon resource."
author:
...
abstract: |
  ...
  bulk transcriptome data by using DoRothEA's curated regulons with various
  statistics
...
on http://bioconductor.org/ and https://github.com/christianholland/dorothea. 
...
Similiar to the `r Biocpkg("viper")` vignette we use the gene expression matrix from the `r Biocpkg("bcellViper")` package.
Click [here](https://bioconductor.org/packages/release/bioc/vignettes/viper/inst/doc/viper.pdf) for more information about the gene expression matrix.
The regulons from DoRothEA are provided within the dorothea package and can be acessed via the `data()` function. As the gene expression matrix contains human data we also load the human version of DoRothEA.
```{r}
...
input types  such as matrix, dataframe, ExpressionSet, Seurat (see dedicated 
vignette for single-cell analysis). We subset DoRothEA to the confidence levels 
A and B to include only the high quality regulons.
